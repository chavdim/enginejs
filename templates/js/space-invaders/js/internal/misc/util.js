...
function CollisionHandler(name, objList1, objList2, parent, onCollisionEvents, collisionDetectionFunction) {
  this.update = function() {
      for (var i = objList1().length - 1; i >= 0; i--) {
        ...
        for (var ii = objList2().length - 1; ii >= 0; ii--) {
          ...
          if (collisionDetectionFunction(o1.rect, o2.rect)) {
            for (let j = 0; j < onCollisionEvents.length; j++) {
              onCollisionEvents[j].apply(o1, o2);
            }
            break;
          }
        }

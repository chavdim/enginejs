function AnimatedImage(imgSource, frames, frameRate, displaceX, displaceY, speedX, speedY) {
  ...
  this.draw = function(ctx, x, y) {
      ...
      let imgx = this.currentFrame * this.widthPerFrame;
      let imgy = 0;
      //width and height cut off from original image
      let imgw = this.widthPerFrame;
...
function BackgroundVisualObject(imgSource, animatedImages, x, y) {
  ...
  this.update = function() {
      ...
      for (let i = this.animatedImages.length - 1; i >= 0; i--) {
...
function FadeTransition(width, height, fadeoutSpeed, fadeinSpeed) {
  ...
  this.draw = function(ctx) {
      let originalStyle = ctx.fillStyle;
      ctx.fillStyle = 'black';
  ...
  this.activate = function(obj, callback) {
      this.obj = obj;
